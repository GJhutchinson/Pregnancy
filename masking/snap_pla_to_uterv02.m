function [pos_store] = snap_pla_to_uterv02(pos_store,selected_mask,slice_n,vol_n,obj_n)
...
% uter_tmp = drawpolyline(im_axes,'position',[pos_store(selected_mask).slice(slice_n).volume(vol_n).object(obj_n).roi(1).Position;pos_store(selected_mask).slice(slice_n).volume(vol_n).object(obj_n).roi(1).Position(1,:)]);
f1 = figure('visible','off');
imagesc(zeros([256,256]))
