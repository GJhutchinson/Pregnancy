function [pla_out,uter_out] = uter_placenta_stats_for_figures(pos_store,uter_ID,slice_n,vol_n)
...
%     
    ...
    if  n_pla+1 ~= uter_ID
        if no_wrap_l(n_pla)<wrap_l(n_pla)
            ...
            pla_out.uterine_outline = no_wrap(n_pla);
        else
            if pla_intersect(2)>pla_intersect(1)
            ...
            pla_out.uterine_outline = wrap(n_pla);
        end
...
uter_out.length =  sum(sqrt(sum(((uterus_pos_tmp(2:end,:)-uterus_pos_tmp(1:end-1,:)).^2),2)));
uter_out.area = polyarea(uterus_pos_tmp(:,1),uterus_pos_tmp(:,2));
