...
S = double(niftiread('R:\DRS-Pip-Ox\original data\035\IVIM\PiP_Ox_035_WIP_DWI_19bvalues_7_1.nii'));
mask = niftiread('D:\Placental\PiP-Ox\data\Scans\035\IVIM\PiP_Ox_035_WIP_DWI_19bvalues_7_1_PF_NewPipeline.nii');
...
parfor n = 1:length(x)
    ...
    %[S0 fIVIM D D*]
    x0_bi_exp = [mono_fast(n,1) 0.4 mono_slow(n,2) mono_fast(n,2)];
    IVIM(n,:) = lsqcurvefit(bi_exp,x0_bi_exp,b,double(S),lb_bi_exp,ub_bi_exp,options);
end
